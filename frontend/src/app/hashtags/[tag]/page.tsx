...
export default function HashtagPage() {
  ...
  const fetchWaves = async () => {
    ...
    setLoading(true);
    try {
      const res = await apiRequest(`/hashtags/${tag}/waves`);
      if (res.ok) {
        ...
        <div className="flex-1 p-4 md:p-6 space-y-6 pb-20 md:pb-6">
          {loading ? (
            ...
            <div className="text-center py-16 space-y-3">
              ...
              <h3 className="text-sm font-bold text-text-secondary">No waves yet</h3>
              <p className="text-xs text-text-muted max-w-xs mx-auto">
                Be the first to create a wave with #{tag}!
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {waves.map((wave) => (
                <WaveCard key={wave.id} wave={wave} onRefresh={fetchWaves} />
              ))}
            </div>
          )}
        </div>
